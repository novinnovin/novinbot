...
utl.get_params_pids_by_full_script_name(
    script_names=[f"{directory}/{filename}"], is_kill_proccess=True
)
print(f"ok: {filename}")
    ...
    if from_id in utl.admins or row_user["status"] == 1:
        if ex_data[0] == "pg":
            if ex_data[1] == "accounts":
                ...
                i = selected_pages + 1
                cs.execute(
                    f"SELECT * FROM {utl.mbots} ORDER BY id DESC LIMIT {selected_pages},{utl.step_page}"
                )
                result = cs.fetchall()
                if not result:
                ...
                for row in result:
                    ...
                    output += (
                        f"📂 دسته بندی: ‏/category_{row['id']} ‏({row_cats['name']})\n"
                    )
            ...
            if ex_data[1] == "0":
                ...
                i = selected_pages + 1
                cs.execute(
                    f"SELECT * FROM {utl.mbots} WHERE status=0 AND user_id IS NOT NULL ORDER BY last_order_at DESC LIMIT {selected_pages},{utl.step_page}"
                )
                result = cs.fetchall()
                if not result:
                    return query.answer(text="⚠️ صفحه دیگری وجود ندارد", show_alert=True)
            ...
            if ex_data[1] == "1":
                ...
                i = selected_pages + 1
                cs.execute(
                    f"SELECT * FROM {utl.mbots} WHERE status=1 AND user_id IS NOT NULL ORDER BY last_order_at ASC LIMIT {selected_pages},{utl.step_page}"
                )
                result = cs.fetchall()
                if not result:
                    return query.answer(text="⚠️ صفحه دیگری وجود ندارد", show_alert=True)
            ...
            if ex_data[1] == "2":
                ...
                i = selected_pages + 1
                cs.execute(
                    f"SELECT * FROM {utl.mbots} WHERE status=2 AND user_id IS NOT NULL ORDER BY end_restrict ASC LIMIT {selected_pages},{utl.step_page}"
                )
                result = cs.fetchall()
                if not result:
                    return query.answer(text="⚠️ صفحه دیگری وجود ندارد", show_alert=True)
            ...
            if ex_data[1] == "orders":
                ...
                i = selected_pages + 1
                cs.execute(
                    f"SELECT * FROM {utl.orders} ORDER BY id DESC LIMIT {selected_pages},{utl.step_page}"
                )
                result = cs.fetchall()
                if not result:
                    return query.answer(text="⚠️ صفحه دیگری وجود ندارد", show_alert=True)

                now = jdatetime.datetime.now().astimezone(
                ...
                cs.execute(
                    f"SELECT COUNT(*) as count FROM {utl.orders} WHERE created_at>={time_today}"
                )
                orders_count_today = cs.fetchone()["count"]
                cs.execute(
                    f"SELECT COUNT(*) as count FROM {utl.orders} WHERE created_at<{time_today} AND created_at>={time_yesterday}"
                )
                orders_count_yesterday = cs.fetchone()["count"]

                cs.execute(
                    f"SELECT COUNT(*) as count FROM {utl.reports} WHERE status=1"
                )
                orders_count_moved_all = cs.fetchone()["count"]
                cs.execute(
                    f"SELECT COUNT(*) as count FROM {utl.reports} WHERE status=1 AND created_at>={time_today}"
                )
                orders_count_moved_today = cs.fetchone()["count"]
                cs.execute(
                    f"SELECT COUNT(*) as count FROM {utl.reports} WHERE status=1 AND created_at<{time_today} AND created_at>={time_yesterday}"
                )
                ...
                for row in result:
                    origin = (
                        f"<a href='{row['origin']}'>{row['origin'].replace('https://t.me/', '')}</a>"
                        if row["origin"] != "0"
                        else "با فایل انجام شده"
                    )
            ...
            if ex_data[1] == "categories":
                ...
                i = selected_pages + 1
                cs.execute(
                    f"SELECT * FROM {utl.cats} ORDER BY id DESC LIMIT {selected_pages},{utl.step_page}"
                )
                result = cs.fetchall()
                if not result:
                    return query.answer(text="⚠️ صفحه دیگری وجود ندارد", show_alert=True)
                    ...
                    for row in result:
                        cs.execute(
                            f"SELECT COUNT(*) as count FROM {utl.mbots} WHERE cat_id={row['id']}"
                        )
                    ...
                    ob = utl.Pagination(
                        update, "categories", output, utl.step_page, rowcount
                    )
                    return ob.process()
            if ex_data[1] == "apis":
                selected_pages = (int(ex_data[2]) - 1) * utl.step_page
                i = selected_pages + 1
                cs.execute(
                    f"SELECT * FROM {utl.apis} ORDER BY id DESC LIMIT {selected_pages},{utl.step_page}"
                )
                result = cs.fetchall()
                if not result:
                    return query.answer(text="⚠️ صفحه دیگری وجود ندارد", show_alert=True)
        ...
        if ex_data[0] == "settings":
            if ex_data[1] == "account_password":
                cs.execute(
                    f"UPDATE {utl.users} SET step='{ex_data[0]};{ex_data[1]}' WHERE user_id={from_id}"
                )
            ...
            return message.edit_reply_markup(
                reply_markup={
                    "inline_keyboard": [
                        [
                            {
                                "text": f"📝 در هر API چند اکانت ثبت شود: {row_admin['api_per_number']} اکانت",
                                "callback_data": "settings;api_per_number",
                            }
                        ],
                        [
                            {
                                "text": f"📝 اد هر اکانت در هر استفاده: {row_admin['add_per_h']} اد",
                                "callback_data": "settings;add_per_h",
                            }
                        ],
                        [
                            {
                                "text": (
                                    f"📝 استفاده اکانت هر چند ساعت: "
                                    + (
                                        f"{int(row_admin['limit_per_h'] / 3600)} ساعت"
                                        if row_admin["limit_per_h"] > 0
                                        else "غیرفعال ❌"
                                    )
                                ),
                                "callback_data": "settings;limit_per_h",
                            }
                        ],
                        [
                            {
                                "text": f"🔐 رمز دو مرحله ای: "
                                + (
                                    row_admin["account_password"]
                                    if row_admin["account_password"] is not None
                                    else "ثبت نشده"
                                )
                                + "",
                                "callback_data": "settings;account_password",
                            }
                        ],
                        [
                            {
                                "text": (
                                    "تنظیم / تغییر رمز دو مرحله ای: "
                                    + (
                                        "فعال ✅"
                                        if row_admin["change_pass"] > 0
                                        else "غیرفعال ❌"
                                    )
                                ),
                                "callback_data": "settings;change_pass",
                            }
                        ],
                        [
                            {
                                "text": (
                                    ...
                                    + (
                                        "فعال ✅"
                                        if row_admin["exit_session"] > 0
                                        else "غیرفعال ❌"
                                    )
                                ),
                                "callback_data": "settings;exit_session",
                            }
                        ],
                        [
                            {
                                "text": (
                                    ...
                                    + (
                                        "فعال ✅"
                                        if row_admin["is_change_profile"] > 0
                                        else "غیرفعال ❌"
                                    )
                                ),
                                "callback_data": "settings;is_change_profile",
                            }
                        ],
                        [
                            {
                                "text": (
                                    ...
                                    + (
                                        "فعال ✅"
                                        if row_admin["is_set_username"] > 0
                                        else "غیرفعال ❌"
                                    )
                                ),
                                "callback_data": "settings;is_set_username",
                            }
                        ],
                    ]
                }
        ...
        if ex_data[0] == "change_status":
            ...
            if row_orders["status"] == 1:
                        ...
                        reply_markup={
                            "inline_keyboard": [
                                ...
                                [
                                    {
                                        "text": "❌ لغو ❌",
                                        "callback_data": f"update;{row_orders['id']}",
                                    },
                                    {
                                        "text": "✅ پایان ✅",
                                        "callback_data": f"{ex_data[0]};{ex_data[1]};2",
                                    },
                                ],
                            ]
                        }
                ...
                if ex_data[2] == "2":
                    row_orders["status"] = 2
                    cs.execute(
                        f"UPDATE {utl.orders} SET status={row_orders['status']} WHERE id={row_orders['id']}"
                    )
            return message.edit_reply_markup(
                reply_markup={
                    "inline_keyboard": [
                        [
                            {
                                "text": utl.status_order[row_orders["status"]],
                                "callback_data": "nazan",
                            }
                        ],
                        [
        ...
        if ex_data[0] == "d":
            ...
            if ex_data[2] == "1" or (
                (ex_data[2] == "0" or ex_data[2] == "2")
                and row_user_select["status"] == 1
            ):
                if from_id in utl.admins:
                    cs.execute(
                        f"UPDATE {utl.users} SET status='{ex_data[2]}' WHERE user_id={row_user_select['user_id']}"
                    )
                else:
                    return query.answer(
                        text="⛔️ این عملیات مخصوص ادمین اصلی است", show_alert=True
                    )
                ...
                cs.execute(
                    f"UPDATE {utl.users} SET step='sendmsg;{row_user_select['user_id']}' WHERE user_id={from_id}"
                )
                return message.reply_html(
            ...
            cs.execute(
                f"SELECT * FROM {utl.users} WHERE user_id={row_user_select['user_id']}"
            )
            row_user_select = cs.fetchone()
                ...
                reply_markup={
                    "inline_keyboard": [
                        [
                            {
                                "text": "ارسال پیام",
                                "callback_data": f"d;{row_user_select['user_id']};sendmsg",
                            }
                        ],
                        [
                            {
                                "text": (
                                    "ادمین ✅"
                                    if row_user_select["status"] == 1
                                    else "ادمین ❌"
                                ),
                                "callback_data": f"d;{row_user_select['user_id']};{admin_status}",
                            }
                        ],
                    ]
                },
            ...
            if row_orders["status"] == 1:
                inline_keyboard.append(
                    [
                        {
                            "text": utl.status_order[row_orders["status"]],
                            "callback_data": f"change_status;{row_orders['id']};end",
                        }
                    ]
                )
            else:
        ...
        if ex_data[0] == "analyze":
            ...
            return message.edit_reply_markup(
                reply_markup={
                    "inline_keyboard": [
                        [{"text": "در حال اتمام ...", "callback_data": "nazan"}]
                    ]
                }
            )
        if ex_data[0] == "is_analyzing":
            ...
            cs.execute(
                f"UPDATE {utl.orders} SET is_analyzing=0 WHERE id={row_orders['id']}"
            )
        ...
        if ex_data[0] == "gc":
            if ex_data[1] == "1":
                ...
                for row_mbots in result:
                    try:
                        cs.execute(
                            f"DELETE FROM {utl.mbots} WHERE id={row_mbots['id']}"
                        )
                        os.remove(
                            f"{directory}/sessions/{row_mbots['uniq_id']}.session"
                        )
                    except:
                        pass
                return message.reply_html(
                    text=f"✅ {len(result)} اکانت لاگ اوت شده حذف شدند"
                )
            if ex_data[1] == "2":
                cs.execute(
                    f"SELECT * FROM {utl.mbots} WHERE status>0 AND ({timestamp}-last_leave_at)>86400"
                )
                result = cs.fetchall()
                if not result:
                    return query.answer(
                        text="⛔️ هیچ اکانتی یافت نشد، اکانت 24 ساعت یکبار می تواند آنالیز شود",
                        show_alert=True,
                    )
                ...
                for row_mbots in result:
                    count_analyze += 1
                    os.system(
                        f"{utl.python_version} \"{directory}/tl_leave.py\" {row_mbots['uniq_id']} {from_id} channel {info_msg.message_id}"
                    )
                    info_msg.edit_text(
...
def private_process(
    ...
    if row_user is None:
        cs.execute(
            f"INSERT INTO {utl.users} (user_id,status,step,prev_step,created_at,uniq_id) VALUES ({from_id},0,'start','start',{timestamp},'{utl.unique_id()}')"
        )
        cs.execute(f"SELECT * FROM {utl.users} WHERE user_id={from_id}")
        row_user = cs.fetchone()
    ...
    if from_id in utl.admins or row_user["status"] == 1:
        if text == "/start" or text == "/panel" or text == utl.menu_var:
            ...
            return cs.execute(
                f"DELETE FROM {utl.orders} WHERE user_id={from_id} AND status=0"
            )
        if text == "/restart":
        ...
        if ex_step[0] == "info_user":
            try:
                ...
                return message.reply_html(
                    text="❌ ورودی نامعتبر", reply_to_message_id=message_id
                )
        ...
        if ex_step[0] == "sendmsg":
            ...
            if row_user_select is None:
                return message.reply_html(
                    text="❌ کاربر یافت نشد", reply_to_message_id=message_id
                )
            if (
                not message.text
                ...
                return message.reply_html(
                    text="⛔️ پیام پشتیبانی نمی شود", reply_to_message_id=message_id
                )
            try:
                ...
                cs.execute(
                    f"UPDATE {utl.users} SET step='panel' WHERE user_id={from_id}"
                )
                return user_panel(
                    message=message,
                ...
                return message.reply_html(
                    text="❌ خطای ناشناخته، مجدد تلاش کنید",
                    reply_to_message_id=message_id,
                )
        if ex_step[0] == "add_api":
            try:
                ...
                if (
                    len(ex_nl_text) != 2
                    or len(ex_nl_text[0]) > 50
                    or len(ex_nl_text[1]) > 200
                ):
                    return message.reply_html(
                        text="❌ ورودی اشتباه است", reply_to_message_id=message_id
                    )
                if not re.findall("^[0-9]*$", ex_nl_text[0]):
                    return message.reply_html(
                        text="‏❌ api id اشتیاه است", reply_to_message_id=message_id
                    )
                if not re.findall("^[0-9-a-z-A-Z]*$", ex_nl_text[1]):
                    return message.reply_html(
                        text="‏❌ api hash اشتیاه است", reply_to_message_id=message_id
                    )

                api_id = ex_nl_text[0]
                api_hash = ex_nl_text[1]
                cs.execute(
                    f"SELECT * FROM {utl.apis} WHERE api_id='{api_id}' OR api_hash='{api_hash}'"
                )
                if cs.fetchone() is not None:
                    return message.reply_html(
                        text="❌ این API قبل افزوده شده است",
                        reply_to_message_id=message_id,
                    )

                cs.execute(
                    f"INSERT INTO {utl.apis} (api_id,api_hash) VALUES ('{api_id}','{api_hash}')"
                )
                return message.reply_html(
                ...
                return message.reply_html(
                    text="❌ ورودی اشتباه", reply_to_message_id=message_id
                )
        if ex_step[0] == "create_cat":
            ...
            if row_cats is not None:
                return message.reply_html(
                    text="❌ دسته بندی قبلا ایجاد شده است",
                    reply_to_message_id=message_id,
                )
        ...
        if ex_step[0] == "set_cat":
            ...
            if row_mbots is None:
                return message.reply_html(
                    text="❌ اکانت یافت نشد", reply_to_message_id=message_id
                )
            cs.execute(f"SELECT * FROM {utl.cats} WHERE name='{text}'")
            row_cats = cs.fetchone()
            if row_cats is None:
                return message.reply_html(
                    text="❌ دسته بندی یافت نشد", reply_to_message_id=message_id
                )

            cs.execute(f"UPDATE {utl.users} SET step='start' WHERE user_id={from_id}")
            cs.execute(
                f"UPDATE {utl.mbots} SET cat_id={row_cats['id']} WHERE id={row_mbots['id']}"
            )
            return message.reply_html(
        ...
        if ex_step[0] == "analyze":
            if ex_step[1] == "type":
                if text == "کاربران":
                    cs.execute(
                        f"UPDATE {utl.users} SET step='analyze;users;link' WHERE user_id={from_id}"
                    )
                    return message.reply_html(
                ...
                if text == "پیام ها":
                    cs.execute(
                        f"UPDATE {utl.users} SET step='analyze;messages;link' WHERE user_id={from_id}"
                    )
                    return message.reply_html(
                        text="لینک گروه را ارسال کنید:",
                        reply_markup={
                ...
                return message.reply_html(
                    text="⛔️ از منو انتخاب کنید", reply_to_message_id=message_id
                )
            if ex_step[1] == "users":
                if ex_step[2] == "link":
                    cs.execute(
                        f"SELECT * FROM {utl.mbots} WHERE status>0 ORDER BY RAND()"
                    )
                    row_mbots = cs.fetchone()
                    if row_mbots is None:
                        return message.reply_html(
                    ...
                    try:
                        int(text)
                        cs.execute(
                            f"INSERT INTO {utl.egroup} (type,user_id,chat_id,status,created_at,updated_at,uniq_id) VALUES (0,{from_id},'{text}',0,{timestamp},{timestamp},'{uniq_id}')"
                        )
                    except:
                        text = text.replace("/+", "/joinchat/")
                        cs.execute(
                            f"INSERT INTO {utl.egroup} (type,user_id,link,status,created_at,updated_at,uniq_id) VALUES (0,{from_id},'{text}',0,{timestamp},{timestamp},'{uniq_id}')"
                        )
                    ...
                    cs.execute(
                        f"UPDATE {utl.users} SET step='{ex_step[0]};{ex_step[1]};account;{row_egroup['id']}' WHERE user_id={from_id}"
                    )
                    return message.reply_html(
                    ...
                    if text == "اکانت رندوم":
                        ...
                        cs.execute(
                            f"SELECT * FROM {utl.mbots} WHERE status>0 AND user_id={int(text)}"
                        )
                    row_mbots = cs.fetchone()
                    if row_mbots is None:
                        return message.reply_html(
                    ...
                    cs.execute(
                        f"UPDATE {utl.users} SET step='start' WHERE user_id={from_id}"
                    )
                    info_msg = message.reply_html(
                        text="در حال اتصال ...", reply_to_message_id=message_id
                    )
                    os.system(
                        f"{utl.python_version} \"{directory}/tl_analyze.py\" {row_mbots['uniq_id']} {from_id} {row_egroup['id']} users {info_msg.message_id}"
                    )
                    user_panel(message=message)
                    return info_msg.delete()
            if ex_step[1] == "messages":
                if ex_step[2] == "link":
                    ...
                    try:
                        int(text)
                        cs.execute(
                            f"INSERT INTO {utl.egroup} (type,user_id,chat_id,status,created_at,updated_at,uniq_id) VALUES (1,{from_id},'{text}',0,'{timestamp}','{timestamp}','{uniq_id}')"
                        )
                    except:
                        text = text.replace("/+", "/joinchat/")
                        cs.execute(
                            f"INSERT INTO {utl.egroup} (type,user_id,link,status,created_at,updated_at,uniq_id) VALUES (1,{from_id},'{text}',0,'{timestamp}','{timestamp}','{uniq_id}')"
                        )
                    cs.execute(f"SELECT * FROM {utl.egroup} WHERE uniq_id='{uniq_id}'")
                    row_egroup = cs.fetchone()
                    if row_egroup is None:
                    ...
                    os.system(
                        f"{utl.python_version} \"{directory}/tl_analyze.py\" {row_mbots['uniq_id']} {from_id} {row_egroup['id']} messages {info_msg.message_id}"
                    )
                    user_panel(message=message)
                    return info_msg.delete()
        ...
        if ex_step[0] == "add_acc":
            ...
            if ex_step[2] == "type":
                if text == "شماره":
                    cs.execute(
                        f"UPDATE {utl.users} SET step='{ex_step[0]};{row_mbots['id']};number;phone' WHERE user_id={from_id}"
                    )
                    return message.reply_html(
                ...
                if text == "سشن":
                    cs.execute(
                        f"UPDATE {utl.users} SET step='{ex_step[0]};{row_mbots['id']};session' WHERE user_id={from_id}"
                    )
                    return message.reply_html(
                ...
                if text == "زیپ":
                    cs.execute(
                        f"UPDATE {utl.users} SET step='{ex_step[0]};{row_mbots['id']};zip' WHERE user_id={from_id}"
                    )
                    return message.reply_html(
            ...
            if ex_step[2] == "session":
                if (
                    ...
                    or message.document.file_name[-8:] != ".session"
                ):
                    return message.reply_html(
                        text="❌ فایل باید از نوع سشن تلتون باشد",
                        reply_to_message_id=message_id,
                    )
                row_apis = utl.select_api(cs, row_admin["api_per_number"])
                if row_apis is None:
                    return message.reply_html(
                        text="❌ ابتدا یک API اضافه کنید یا از تنظیمات گزینه اول را افزایش دهید",
                        reply_to_message_id=message_id,
                    )
                try:
                    unique_id = utl.unique_id()
                    cs.execute(
                        f"INSERT INTO {utl.mbots} (cat_id,creator_user_id,api_id,api_hash,status,created_at,uniq_id) VALUES (1,{from_id},'{row_apis['api_id']}','{row_apis['api_hash']}',0,{int(time.time())},'{unique_id}')"
                    )
                    ...
                    with open(
                        f"{directory}/sessions/{row_mbots_select['uniq_id']}.session",
                        "wb",
                    ) as file:
                        file.write(requests.get(info_action.file_path).content)
                    info_msg = message.reply_html(text="در حال بررسی ...")
                    os.system(
                        f"{utl.python_version} \"{directory}/tl_import.py\" {row_mbots_select['uniq_id']}"
                    )
                    cs.execute(
                        f"SELECT * FROM {utl.mbots} WHERE id={row_mbots_select['id']}"
                    )
                    row_mbots_select = cs.fetchone()
                    if row_mbots_select is not None:
                        if row_mbots_select["status"] == 1:
                            return info_msg.edit_text(
                                text=f"✅ ذخیره شد: <code>{row_mbots_select['phone']}</code>",
                                parse_mode="html",
                            )
                        else:
                            cs.execute(
                                f"DELETE FROM {utl.mbots} WHERE id={row_mbots_select['id']}"
                            )
                            return info_msg.edit_text(
                                text=f"❕ قبلا اضافه شده: <code>{row_mbots_select['phone']}</code>",
                                parse_mode="html",
                            )
                    else:
            ...
            if ex_step[2] == "zip":
                cs.execute(
                    f"DELETE FROM {utl.mbots} WHERE creator_user_id={from_id} AND status=0 AND user_id IS NULL"
                )
                if not message.document or message.document.file_name[-4:] != ".zip":
                    return message.reply_html(
                        text="❌ فایل باید از نوع زیپ فایل",
                        reply_to_message_id=message_id,
                    )
                try:
                    try:
                    ...
                    if not os.path.exists(f"{directory}/import"):
                        os.mkdir(f"{directory}/import")
                    info_msg = message.reply_html(
                        text="در حال دانلود ...", reply_to_message_id=message_id
                    )
                    info_action = bot.get_file(message.document.file_id)
                    ...
                    count_import_success = count_import_failed = (
                        count_import_existed
                    ) = 0
                    for file in list_files:
                        ...
                        if file[-8:] == ".session":
                            try:
                                ...
                                cs.execute(
                                    f"SELECT * FROM {utl.mbots} WHERE uniq_id='{unique_id}'"
                                )
                                row_mbots = cs.fetchone()
                                ...
                                with open(
                                    ...
                                    file.write(content)
                                os.system(
                                    f"{utl.python_version} \"{directory}/tl_import.py\" {row_mbots['uniq_id']}"
                                )
                                cs.execute(
                                    f"SELECT * FROM {utl.mbots} WHERE id={row_mbots['id']}"
                                )
                                row_mbots = cs.fetchone()
            ...
            if ex_step[2] == "number":
                if ex_step[3] == "phone":
                    ...
                    if not re.findall("^[0-9]*$", phone):
                        return message.reply_html(
                            text="❌ شماره اشتباه است", reply_to_message_id=message_id
                        )

                    cs.execute(
                        f"SELECT * FROM {utl.mbots} WHERE phone='{phone}' AND status>0"
                    )
                    row_mbots_select = cs.fetchone()
                    if row_mbots_select is not None:
                        return message.reply_html(
                            text="❌ شماره قبلا اضافه شده است",
                            reply_to_message_id=message_id,
                        )
                    cs.execute(
                        f"UPDATE {utl.mbots} SET phone='{phone}' WHERE id={row_mbots['id']}"
                    )
                    info_msg = message.reply_html(
                        text="در حال اتصال ...", reply_to_message_id=message_id
                    )
                    os.system(
                        f"{utl.python_version} \"{directory}/tl_account.py\" {row_mbots['uniq_id']} {from_id} {info_msg.message_id}"
                    )
                    return info_msg.delete()
                if ex_step[3] == "code":
                    try:
                        ...
                        pass
                    return cs.execute(
                        f"UPDATE {utl.mbots} SET code={code} WHERE id={row_mbots['id']}"
                    )
                if ex_step[3] == "password":
                    return cs.execute(
                        f"UPDATE {utl.mbots} SET password='{text}' WHERE id={row_mbots['id']}"
                    )
        if ex_step[0] == "create_order":
            ...
            if ex_step[1] == "category":
                if text == "⏩ بعدی":
                    if row_orders["cats"] is None:
                        return message.reply_html(
                            text="❌ حداقل باید یک دسته بندی انتخاب کنید",
                            reply_to_message_id=message_id,
                        )
                    cs.execute(
                        f"UPDATE {utl.users} SET step='create_order;type_analyze;{row_orders['id']}' WHERE user_id={from_id}"
                    )
                    return message.reply_html(
                    ...
                    if row_cats is None:
                        ...
                        if row_orders["cats"] is not None:
                            ...
                            for category in cats:
                                try:
                                    if int(category) == row_cats["id"]:
                                        return message.reply_html(
                                            text=f"❌ دسته بندی <b>{row_cats['name']}</b> قبلا انتخاب شده است",
                                            reply_to_message_id=message_id,
                                        )
                                except:
                                    pass
                            ...
                            cats = row_cats["id"]
                        cs.execute(
                            f"UPDATE {utl.orders} SET cats='{cats}' WHERE id={row_orders['id']}"
                        )
            ...
            if ex_step[1] == "type_analyze":
                ...
                cs.execute(
                    f"UPDATE {utl.orders} SET type_analyze={type_analyze} WHERE id={row_orders['id']}"
                )
                cs.execute(
                    f"UPDATE {utl.users} SET step='create_order;type;{row_orders['id']}' WHERE user_id={from_id}"
                )
                return message.reply_html(
            ...
            if ex_step[1] == "type":
                if text == "🔴 لینک گروه 🔴":
                    cs.execute(
                        f"UPDATE {utl.users} SET step='create_order;info;{row_orders['id']}' WHERE user_id={from_id}"
                    )
                    return message.reply_html(
                ...
                if text == "🔵 لیست اعضا 🔵":
                    cs.execute(
                        f"UPDATE {utl.users} SET step='create_order_file;info;{row_orders['id']}' WHERE user_id={from_id}"
                    )
                    return message.reply_html(
                        text="مطابق نمونه ارسال کنید:\n\n"
            ...
            if ex_step[1] == "info":
                ...
                try:
                    ...
                    ex_nl_text = text.strip().split("\n")

                    # آخرین خط: تعداد ارسال
                    count = int(ex_nl_text[-1].strip())

                    # ماقبل آخرین خط: لینک مقصد
                    target = ex_nl_text[-2].strip().replace("/+", "/joinchat/")
                    ...
                    for src in sources:
                        if not src.startswith("https://t.me/"):
                            return message.reply_html(
                                text=f"❌ لینک مبدا نامعتبر است: {src}", reply_to_message_id=message_id
                            )

                except Exception as e:
                    return message.reply_html(text=f"❌ ورودی اشتباه است: {str(e)}", reply_to_message_id=message_id)

                # ذخیره اطلاعات در دیتابیس
                sources_str = ",".join(sources)  # تبدیل لیست به رشته جدا شده با کاما
                cs.execute(
                    f"UPDATE {utl.orders} SET origin='{sources_str}', destination='{target}', count={count}, is_analyzing=1 WHERE id={row_orders['id']}"
                )
                cs.execute(f"UPDATE {utl.users} SET step='start' WHERE user_id={from_id}")
                ...
                os.system(
                    f"{utl.python_version} \"{directory}/tl_analyze.py\" {row_mbots['uniq_id']} {from_id} {row_orders['id']} order_link {info_msg.message_id}"
                )
                return info_msg.delete()
            ...
            if ex_step[1] == "type_users":
                if text == "همه کاربران":
                    ...
                    type_users = 1
                    cs.execute(
                        f"DELETE FROM {utl.reports} WHERE order_id={row_orders['id']} AND is_real=0"
                    )
                elif text == "کاربران فیک":
                    type_users = 2
                    cs.execute(
                        f"DELETE FROM {utl.reports} WHERE order_id={row_orders['id']} AND is_real=1"
                    )
                elif text == "کاربران آنلاین":
                    type_users = 3
                    cs.execute(
                        f"DELETE FROM {utl.reports} WHERE order_id={row_orders['id']} AND is_online=0"
                    )
                elif text == "کاربران با شماره":
                    type_users = 4
                    cs.execute(
                        f"DELETE FROM {utl.reports} WHERE order_id={row_orders['id']} AND is_withphone=0"
                    )
                else:
                    return message.reply_html(
                        text="❌ از منو انتخاب کنید", reply_to_message_id=message_id
                    )
                cs.execute(
                    f"SELECT COUNT(*) as count FROM {utl.reports} WHERE order_id={row_orders['id']}"
                )
                max_users = cs.fetchone()["count"]
                cs.execute(
                    f"UPDATE {utl.orders} SET status=1,max_users={max_users},type_users={type_users},add_per_h={row_admin['add_per_h']},created_at={timestamp},updated_at={timestamp} WHERE id={row_orders['id']}"
                )
                cs.execute(
                    f"UPDATE {utl.users} SET step='start' WHERE user_id={from_id}"
                )
                return user_panel(
                    message=message,
                    text=f"✅ سفارش ایجاد شد: /order_{row_orders['id']}",
                    reply_to_message_id=message_id,
                )
        if ex_step[0] == "create_order_file":
            ...
            if ex_step[1] == "info":
                ...
                cs.execute(
                    f"UPDATE {utl.orders} SET destination='{target}',count={count},is_analyzing=1 WHERE id={row_orders['id']}"
                )
                cs.execute(
                    f"UPDATE {utl.users} SET step='start' WHERE user_id={from_id}"
                )
                ...
                os.system(
                    f"{utl.python_version} \"{directory}/tl_analyze.py\" {row_mbots['uniq_id']} {from_id} {row_orders['id']} order_file {info_msg.message_id}"
                )
                return info_msg.delete()
            if ex_step[1] == "file":
                cs.execute(f"SELECT * FROM {utl.orders} WHERE id={int(ex_step[2])}")
                ...
                if not message.document:
                    return message.reply_html(
                        text="❌ یک فایل TXT ارسال کنید", reply_to_message_id=message_id
                    )

                info_msg = message.reply_html(
                    text="در حال بررسی فایل ...", reply_to_message_id=message_id
                )
                try:
                    list_members = []
                    info_action = bot.get_file(message.document.file_id)
                    with open(
                        f"{directory}/files/id-{row_orders['id']}.txt", "wb"
                    ) as file:
                        file.write(requests.get(info_action.file_path).content)
                    with open(
                        f"{directory}/files/id-{row_orders['id']}.txt", "rb"
                    ) as file:
                    ...
                    for value in list_members:
                        utl.insert(
                            cs,
                            f"INSERT INTO {utl.reports} (order_id,username,is_real,status,created_at) VALUES ({row_orders['id']},'{value}',1,0,{timestamp})",
                        )
                    ...
                    count = cs.fetchone()["count"]
                    cs.execute(
                        f"UPDATE {utl.orders} SET status=1,max_users={count},type_users=0,add_per_h='{row_admin['add_per_h']}',created_at='{timestamp}',updated_at='{timestamp}' WHERE id={row_orders['id']}"
                    )
                    cs.execute(
                        f"UPDATE {utl.users} SET step='start' WHERE user_id={from_id}"
                    )
        ...
        if text == "➕ ایجاد سفارش":
            ...
            if row_orders is not None:
                return message.reply_html(
                    text="❌ یک سفارش فعال یافت نشد", reply_to_message_id=message_id
                )

            cs.execute(
                f"SELECT * FROM {utl.mbots} WHERE status=1 AND (last_order_at+{row_admin['limit_per_h']})<{timestamp} ORDER BY last_order_at ASC"
            )
            if cs.fetchone() is None:
                return message.reply_html(
                    text="❌ ابتدا یک اکانت اضافه کنید", reply_to_message_id=message_id
                )

            uniq_id = utl.unique_id()
            cs.execute(
                f"INSERT INTO {utl.orders} (user_id,status,is_finalanalyzed,created_at,updated_at,uniq_id) VALUES ({from_id},0,0,{timestamp},{timestamp},'{uniq_id}')"
            )
            ...
            cs.execute(
                f"UPDATE {utl.users} SET step='create_order;category;{row_orders['id']}' WHERE user_id={from_id}"
            )
            keyboard = [[{"text": utl.menu_var}, {"text": "⏩ بعدی"}]]
            cs.execute(f"SELECT * FROM {utl.cats}")
            result = cs.fetchall()
        ...
        if text == "📋 سفارش ها":
            cs.execute(
                f"SELECT * FROM {utl.orders} WHERE status>0 ORDER BY id DESC LIMIT 0,{utl.step_page}"
            )
            result = cs.fetchall()
            if not result:
                return message.reply_html(
                    text="❌ لیست خالی است", reply_to_message_id=message_id
                )
        ...
        if text == "➕ افزودن اکانت":
            ...
            uniq_id = utl.unique_id()
            cs.execute(
                f"INSERT INTO {utl.mbots} (cat_id,creator_user_id,api_id,api_hash,status,created_at,uniq_id) VALUES (1,{from_id},{row_apis['api_id']},'{row_apis['api_hash']}',0,{timestamp},'{uniq_id}')"
            )
            ...
            cs.execute(
                f"UPDATE {utl.users} SET step='add_acc;{row_mbots['id']};type' WHERE user_id={from_id}"
            )
            return message.reply_html(
        ...
        if text == "📋 اکانت ها":
            cs.execute(
                f"SELECT COUNT(*) as count FROM {utl.mbots} WHERE user_id IS NOT NULL"
            )
            accs_all = cs.fetchone()["count"]
            cs.execute(
                f"SELECT COUNT(*) as count FROM {utl.mbots} WHERE user_id IS NOT NULL AND status=0"
            )
                ...
                reply_markup={
                    "inline_keyboard": [
                        [
                            {
                                "text": f"💢 همه ({accs_all}) 💢",
                                "callback_data": f"pg;accounts;1",
                            }
                        ],
                        [
                            {
                                "text": f"⛔️ لاگ اوت شده ({accs_logout})",
                                "callback_data": f"pg;0;1",
                            },
                            {
                                "text": f"❌ محدود شده ({accs_restrict})",
                                "callback_data": f"pg;2;1",
                            },
                        ],
                        [
                            {
                                "text": f"✅ فعال ({accs_active})",
                                "callback_data": f"pg;1;1",
                            }
                        ],
        ...
        if text == "➕ افزودن API":
            cs.execute(
                f"UPDATE {utl.users} SET step='add_api;' WHERE user_id={from_id}"
            )
            return message.reply_html(
        ...
        if text == "‏📋 API ها":
            cs.execute(
                f"SELECT * FROM {utl.apis} ORDER BY id DESC LIMIT 0,{utl.step_page}"
            )
            result = cs.fetchall()
            if not result:
                return message.reply_html(
                    text="❌ لیست API خالی است", reply_to_message_id=message_id
                )
        ...
        if text == "➕ ایجاد دسته بندی":
            cs.execute(
                f"UPDATE {utl.users} SET step='create_cat;none' WHERE user_id={from_id}"
            )
            return message.reply_html(
        ...
        if text == "📋 دسته بندی ها":
            cs.execute(
                f"SELECT * FROM {utl.cats} ORDER BY id DESC LIMIT 0,{utl.step_page}"
            )
            result = cs.fetchall()
            if not result:
                return message.reply_html(
        ...
        if text == "🔮 آنالیز":
            ...
            cs.execute(
                f"UPDATE {utl.users} SET step='analyze;type' WHERE user_id={from_id}"
            )
            return message.reply_html(
        ...
        if text == "👤 کاربر":
            cs.execute(
                f"UPDATE {utl.users} SET step='info_user;' WHERE user_id={from_id}"
            )
            return message.reply_html(
        ...
        if ex_text[0] == "/order":
            ...
            if row_orders is None:
                return message.reply_html(
                    text="❌ سفارش یافت نشد", reply_to_message_id=message_id
                )
            inline_keyboard = []
            if row_orders["status"] == 1:
                inline_keyboard.append(
        ...
        if ex_text[0] == "/category":
            ...
            cs.execute(
                f"UPDATE {utl.users} SET step='set_cat;{row_mbots['id']}' WHERE user_id={from_id}"
            )
            keyboard = []
            cs.execute(f"SELECT * FROM {utl.cats}")
            result = cs.fetchall()
        ...
        if ex_text[0] == "/DeleteCat":
            ...
            cs.execute(
                f"SELECT COUNT(*) as count FROM {utl.mbots} WHERE cat_id={row_cats['id']}"
            )
            count = cs.fetchone()["count"]
            if count < 1:
                cs.execute(f"DELETE FROM {utl.cats} WHERE id={row_cats['id']}")
                return message.reply_html(
                    text="✅ با موفقیت حذف شد", reply_to_message_id=message_id
                )
        ...
        if ex_text[0] == "/status":
            ...
            return os.system(
                f"{utl.python_version} \"{directory}/tl_account_status.py\" {row_mbots['uniq_id']} {from_id} {info_msg.message_id} check"
            )
        if ex_text[0] == "/delete":
            cs.execute(f"SELECT * FROM {utl.mbots} WHERE id={int(ex_text[1])}")
            row_mbots = cs.fetchone()
        ...
        if ex_text[0] == "/deleteconfirm":
            ...
            return message.reply_html(
                text=f"‏✅ اکانت <code>{row_mbots['phone']}</code> با موفقیت حذف شد",
                reply_to_message_id=message_id,
            )
        if ex_text[0] == "/DeleteApi":
            ...
            if row_apis is None:
                return message.reply_html(
                    text="‏❌ API یافت نشد", reply_to_message_id=message_id
                )

            cs.execute(f"DELETE FROM {utl.apis} WHERE id={row_apis['id']}")
            return message.reply_html(
                text="‏✅ API با موفقیت حذف شد", reply_to_message_id=message_id
            )
        if ex_text[0] == "/ex":
                ...
                try:
                    if ex_text[2] == "a":
                        message.reply_document(
                            document=open(
                                f"{directory}/export/{row_egroup['id']}/users_all.txt",
                                "rb",
                            ),
                            caption="همه کاربران",
                            reply_to_message_id=message_id,
                        ...
                        message.reply_document(
                            document=open(
                                f"{directory}/export/{row_egroup['id']}/users_real.txt",
                                "rb",
                            ),
                            caption="کاربران واقعی",
                            reply_to_message_id=message_id,
                        )
                    elif ex_text[2] == "f":
                        message.reply_document(
                            document=open(
                                f"{directory}/export/{row_egroup['id']}/users_fake.txt",
                                "rb",
                            ),
                            caption="کاربران فیک",
                            reply_to_message_id=message_id,
                        ...
                        message.reply_document(
                            document=open(
                                f"{directory}/export/{row_egroup['id']}/users_has_phone.txt",
                                "rb",
                            ),
                            caption="کاربران با شماره",
                            reply_to_message_id=message_id,
                        )
                    elif ex_text[2] == "o":
                        message.reply_document(
                            document=open(
                                f"{directory}/export/{row_egroup['id']}/users_online.txt",
                                "rb",
                            ),
                            caption="کربران آنلاین",
                            reply_to_message_id=message_id,
                        )
                except:
                ...
                try:
                    if ex_text[2] == "a":
                        message.reply_document(
                            ...
                            caption="کاربارن شناسایی شده",
                            reply_to_message_id=message_id,
                        )
                    elif ex_text[2] == "u":
                        message.reply_document(
                            document=open(
                                f"{directory}/export/{row_egroup['id']}/users_username.txt",
                                "rb",
                            ),
                            caption="کاربران با یوزرنیم",
                            reply_to_message_id=message_id,
                        )
                    elif ex_text[2] == "b":
                        message.reply_document(
                            document=open(
                                f"{directory}/export/{row_egroup['id']}/users_bots.txt",
                                "rb",
                            ),
                            caption="ربات ها",
                            reply_to_message_id=message_id,
        ...
        if ex_text[0] == "/exo":
            ...
            if not row_orders["is_finalanalyzed"]:
                return message.reply_html(
                    text="❌ گزارش سفارش هنوز تکمیل نشده است",
                    reply_to_message_id=message_id,
                )
            info_msg = message.reply_html(text="در حال ارسال ...")
            if ex_text[2] == "m":
                if not os.path.exists(
                    f"{directory}/files/exo_{row_orders['id']}_m.txt"
                ):
                    return message.reply_html(
                        text="❌ هیچ ممبری یافت نشد", reply_to_message_id=message_id
                    )
                message.reply_document(
                    document=open(
                        f"{directory}/files/exo_{row_orders['id']}_m.txt", "rb"
                    ),
                    caption="کاربران منتقل شده",
                    reply_to_message_id=message_id,
                )
            elif ex_text[2] == "e":
                if not os.path.exists(
                    f"{directory}/files/exo_{row_orders['id']}_e.txt"
                ):
                    return message.reply_html(
                        text="❌ هیچ ممبری یافت نشد", reply_to_message_id=message_id
                    )
                message.reply_document(
                    document=open(
                        f"{directory}/files/exo_{row_orders['id']}_e.txt", "rb"
                    ),
                    caption="کاربران منتقل نشده (مشکل دار)",
                    reply_to_message_id=message_id,
                )
            elif ex_text[2] == "r":
                if not os.path.exists(
                    f"{directory}/files/exo_{row_orders['id']}_r.txt"
                ):
                    return message.reply_html(
                        text="❌ هیچ ممبری یافت نشد", reply_to_message_id=message_id
                    )
                message.reply_document(
                    document=open(
                        f"{directory}/files/exo_{row_orders['id']}_r.txt", "rb"
                    ),
                    caption="کاربران منتقل شده",
                    reply_to_message_id=message_id,
                )
...
if __name__ == "__main__":
    ...
    dispatcher.add_handler(
        telegram.ext.MessageHandler(
            ...
            & telegram.ext.Filters.update.message
            & telegram.ext.Filters.update,
            private_process,
            run_async=True,
        )
    )
    dispatcher.add_handler(
        telegram.ext.CallbackQueryHandler(callbackquery_process, run_async=True)
    )
