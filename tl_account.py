...
try:
    ...
    cs.execute(
        f"UPDATE {utl.mbots} SET code=null,password=null WHERE id={row_mbots['id']}"
    )
    cs.execute(
        f"UPDATE {utl.users} SET step='add_acc;{row_mbots['id']};number;code' WHERE user_id={from_id}"
    )
        ...
        if row_mbots is None:
            ...
            if row_mbots["code"] is not None:
                try:
                    ...
                    cs.execute(
                        f"UPDATE {utl.mbots} SET code=null WHERE id={row_mbots['id']}"
                    )
                    ...
                    cs.execute(
                        f"UPDATE {utl.users} SET step='add_acc;{row_mbots['id']};number;password' WHERE user_id={from_id}"
                    )
                    step_login = "password"
            ...
            if row_mbots["password"] is not None:
                try:
                    ...
                    cs.execute(
                        f"UPDATE {utl.mbots} SET password=null WHERE id={row_mbots['id']}"
                    )
                    info_msg.reply_html(text="❌ رمز عبور اشتباه است، مجدد وارد کنید")
        if step_login == "success":
            cs.execute(
                f"UPDATE {utl.mbots} SET user_id={me.id},status=1 WHERE id={row_mbots['id']}"
            )
    ...
    info_msg.reply_html(
        text=f"❌ شماره به مدت {utl.convert_time(e.seconds, 2)} محدود شده است"
    )
