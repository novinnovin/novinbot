...
try:
    ...
    if client.is_user_authorized():
        ...
        for message in client.iter_messages(get_input_entity):
            try:
                ...
                ).astimezone(datetime.timezone(datetime.timedelta(hours=3, minutes=30)))
                regex = re.findall(
                    "Login code: [\d]*. Do not give this code", message.message
                )[0]
                code = regex.replace("Login code: ", "").replace(
                ...
                pass
        password = (
            f"<code>{row_mbots['password']}</code>"
            if row_mbots["password"] is not None
            else "ندارد"
        )
        code = (
            f"<code>{code}</code>\n‏   📅 {code_date.strftime('%Y-%m-%d %H:%M:%S')}"
            if code is not None
            else "ندارد"
        )
        me = client.get_me()
        ...
        info_msg.edit_text(
            ...
            f"   🔻 نام: {me.first_name}\n"
            f"   🔻 نام خانوادگی: "
            + (me.last_name if me.last_name is not None else "ثبت نشده")
            + "\n"
            f"   🔻 یوزرنیم: "
            + (me.username if me.username is not None else "ثبت نشده")
            + "\n"
            f"   🔻 تصویر: {photo}\n"
            f"\nرمز عبور: {password}\n"
    ...
    info_msg.edit_text(
        text=f"❌ اکانت به مدت {utl.convert_time(e.seconds, 2)} محدود شده است"
    )
